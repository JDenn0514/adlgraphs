...
Now let's check this with `x` from the new data. Since `x` has a variable label, it will be a bit more in-depth having to replicate the results we get from the `num_rev()` function. We can see that both variables are the same. Moreover, what was just one line of code ended up being ~5 lines of code. While this may not seem like much, it becomes a lot when you have a bunch of variables to do this over. Now to understand the output from `str(new_df)`, there are three things to note here:
...
`z` is the most complex variable as it contains both variable and value labels. What was originally one line of code quickly became 9 lines of code. 
