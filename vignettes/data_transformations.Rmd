...
Now that it's clear what this function does and what it doesn't do, let's try manually doing it. First, let's try just subtracting 5 from `top`. As we can see all attributes have been stripped.

```{r}
new_df <- test_data %>% 
  # reverse the variable
  mutate(top_rev = 5 - top)

# check attributes
attributes(new_df$top_rev)
...
There is an easier of doing this by using `labelled::val_labels()` and `labelled::var_label()` to leverage the existing value and variable labels from `top`. However, when setting the value labels, we have to use `rev()` to reverse them.
...
new_df <- test_data %>% 
  mutate(
    top_rev = labelled(
      x = 5 - top,
      labels = val_labels(test_data$top) %>% rev(),
      label = var_label(test_data$top)
    )
  )
