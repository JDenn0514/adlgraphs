  ...
  test <- test_data %>% 
    dplyr::mutate(
      new = row_sums(
        ...
        label = "Social Dominance Orientation Sums",
        na.rm = TRUE
      ),
      base = rowSums(dplyr::pick(c(top_rev:deserving_flip)), na.rm = TRUE) %>% 
        structure(
          label = "Social Dominance Orientation Sums",
          variables = c("top_rev", "inferior_rev", "dominate_flip", "deserving_flip"),
          transformation = "Took the sum of top_rev, inferior_rev, dominate_flip, deserving_flip"
        )
    )
