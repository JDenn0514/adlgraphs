testthat::test_that("default: single x, no group, unweighted matches count", {
  ...
  out <- get_freqs(df, x = x1, wt = wts, na.rm = TRUE, drop_zero = FALSE, decimals = 3)
  ...
  # x1 "yes": rows 1 (1), 3 (1), 7 (1) => 3; "no": rows 2 (2), 5 (1), 6 (3), 8 (2) => 8; NA row 4 excluded by na.rm
  # Sum = 11
  yes_n <- out$n[out$x1 == "yes"]
  no_n  <- out$n[out$x1 == "no"]
  testthat::expect_equal(as.numeric(yes_n), 3)
  testthat::expect_equal(as.numeric(no_n), 8)

  # pct rounded to decimals+2 = 5 digits
  # yes: 3/11, no: 8/11
  testthat::expect_equal(out$pct[out$x1 == "yes"], round(3/11, 5))
  testthat::expect_equal(out$pct[out$x1 == "no"], round(8/11, 5))

  # labels attached
  testthat::expect_equal(attr(out$x1, "label"), "Q1. Agree?")
...
testthat::test_that("default: grouping works and respects weights", {
  ...
  out <- get_freqs(df, x = x1, group = grp, wt = wts, na.rm = TRUE, drop_zero = FALSE, decimals = 2)
...
testthat::test_that("default: multi-x without groups pivots and aggregates correctly", {
  ...
  testthat::expect_true(all(unique(out$item) %in% c("Q1. Agree?", "Q2. Color")))

  # For x2 (ignoring NA at row 8), compute weighted counts:
  # red: rows 1(1),2(2),5(1) => 4; blue: rows 3(1),4(1),6(3),7(1) => 6; total 10
  # out_x2 <- subset(out, item == "Q2. Color?")
  testthat::expect_equal(out$n[[4]], 4)
  testthat::expect_equal(out$n[[3]], 6)
  testthat::expect_equal(out$pct[[3]], round(6/10, 5))
})
...
testthat::test_that("default: multi-x with groups works and keep filter applies", {
  ...
  out <- get_freqs(
    ...
    names_to = "item",
    values_to = "resp",
    keep = c("yes", "red"),  # keep filter over resp
    na.rm = TRUE, 
    drop_zero = FALSE, 
    decimals = 3
  )

  testthat::expect_true(all(c("grp", "item", "resp", "n", "pct") %in% names(out)))
  testthat::expect_true(all(out$resp %in% c("yes", "red")))
...
testthat::test_that("default: drop_zero drops zero-count rows", {
  ...
  out_keep_zero <- get_freqs(df, x = x1, wt = wts, na.rm = TRUE, drop_zero = FALSE)
  out_drop_zero <- get_freqs(df, x = x1, wt = wts, na.rm = TRUE, drop_zero = TRUE)
...
testthat::test_that("default: decimals controls rounding as expected", {
  ...
  # For "yes": 3.00 vs 3; pct 0.27273 -> 0.27 vs 0.27 (since pct gets 2 and 2 more)
  testthat::expect_equal(out2$n[out2$x1 == "yes"], 3.00)
  testthat::expect_equal(out0$n[out0$x1 == "yes"], 3)
  testthat::expect_equal(out2$pct[out2$x1 == "yes"], round(3/11, 4))
  testthat::expect_equal(out0$pct[out0$x1 == "yes"], round(3/11, 2))
})
...
testthat::test_that("default: errors and warnings are informative", {
  ...
  # x selects nothing
  testthat::expect_error(get_freqs(df, x = tidyselect::starts_with("zzz")),
                         "x must select at least one column")

  # invalid wt name
  testthat::expect_error(get_freqs(df, x = x1, wt = bad_wt),
  ...
  df_all_na <- df
  df_all_na$x1 <- NA
  testthat::expect_error(get_freqs(df_all_na, x = x1, wt = wts, na.rm = TRUE),
...
testthat::test_that("survey: single x no groups matches svytable", {
  ...
  testthat::expect_true(all(c("x1", "n", "pct") %in% names(out)))
  # Should match default weighted results (3 and 8 from earlier)
  testthat::expect_equal(out$n[out$x1 == "yes"], 3)
  testthat::expect_equal(out$n[out$x1 == "no"], 8)
})
...
testthat::test_that("survey: multi-x no groups pivots and aggregates correctly", {
  ...
  testthat::expect_true(all(c("item", "resp", "n", "pct") %in% names(out)))
  out_x2 <- subset(out, item == "Q2. Color")
  testthat::expect_equal(out_x2$n[[1]], 4)
  testthat::expect_equal(out_x2$n[[2]], 3)
})
...
testthat::test_that("survey: drop_zero drops zero rows and decimals apply", {
  ...
  # Check rounding on pct for yes: 3/11 rounded to 0.27 with decimals=1 -> pct uses 3 places total? In survey path pct = decimals+2 = 3
  testthat::expect_equal(out$pct[out$x1 == "yes"], round(3/11, 3))
})
...
testthat::test_that("low_freqs Case C: error when name_col or value_col not found", {
  ...
  testthat::expect_error(
    adlgraphs:::low_freqs(dsn, value_col = "resp", name_col = "item", values_to = "resp"),
...
testthat::test_that("keep = character filters values in multi-x (data.frame)", {
  ...
  testthat::expect_s3_class(out, "adlgraphs_freqs")
  testthat::expect_named(out, c("item", "resp", "n", "pct"), ignore.order = TRUE)
  testthat::expect_true(all(out$resp %in% "yes"))
  testthat::expect_true(all(out$item %in% c("Q1. Agree?", "Q2. Color")))
})
...
testthat::test_that("keep = function works and TRUE is a no-op (data.frame)", {
  ...
  out_fun <- get_freqs(
    ...
    names_to = "item",
    values_to = "resp",
    keep = function(v) grepl("e$", v), # keeps "blue" if present, otherwise only "no"/"yes" unaffected
    na.rm = TRUE
  )
...
testthat::test_that("keep is ignored for single-x (data.frame)", {
  df <- label_vars(make_basic_df())

  out <- get_freqs(
    df, x = x1, wt = wts, keep = c("yes"), na.rm = FALSE
  )
  # All observed levels appear since keep is ignored in single-x
...
testthat::test_that("keep is ignored for single-x with groups (survey.design) and NA retained", {
  df  <- label_vars(make_basic_df())
  svy <- make_svy(df)
  ...
  out <- get_freqs(
    ...
    keep = c("yes"),      # should be ignored for single-x
...
testthat::test_that("keep filters responses and item labels are applied (multi-x, no groups, survey.design)", {
  ...
  out <- get_freqs(
    ...
    na.rm = FALSE          # retain NA rows (should be kept if present, but filtered out by keep)
  )
...
testthat::test_that("keep filters responses; item labels and groups retained (multi-x with groups, survey.design)", {
  ...
  out <- get_freqs(
    ...
    na.rm = FALSE                      # NA present before filter; filtered out if not matching
  )
...
testthat::test_that("survey Case B: na.rm=TRUE completes zero-count levels by group", {
  ...
  testthat::expect_true(all(vapply(by_grp, function(d) any(d$x1 == "maybe"), logical(1))))
...
testthat::test_that("survey Case C: na.rm toggles NA retention in multi-x no groups", {
  ...
  out_false <- get_freqs(svy, x = c("x1", "x2"), names_to = "item", values_to = "resp", na.rm = FALSE)
  out_true  <- get_freqs(svy, x = c("x1", "x2"), names_to = "item", values_to = "resp", na.rm = TRUE)
...
testthat::test_that("apply_keep_filter character keep can include NA explicitly", {
  ...
  out <- apply_keep_filter(df, values_col = "resp", keep = rlang::quo(c("yes", NA)))
...
testthat::test_that("apply_keep_filter applies function on character values, not factor codes", {
  df <- tibble::tibble(resp = factor(c("yes", "no", "maybe"), levels = c("yes", "no", "maybe")), n = 1:3)
  out <- apply_keep_filter(df, values_col = "resp", keep = rlang::quo(function(v) grepl("^m", v)))
...
testthat::test_that("survey multi-x item column uses variable labels (name_label propagation)", {
  ...
  out <- get_freqs(svy, x = c("x1", "x2"), names_to = "item", values_to = "resp", na.rm = TRUE)
...
testthat::test_that("default single-x ignores keep even with na.rm=TRUE", {
  ...
  # NA removed due to na.rm=TRUE; keep ignored so both yes and no appear
  testthat::expect_true(all(c("yes", "no") %in% out$x1))
})

testthat::test_that("survey multi-x values_to remains factor unless filtered to empty", {
  df  <- label_vars(make_basic_df())
  svy <- make_svy(df)

  out <- get_freqs(svy, x = c("x1", "x2"), names_to = "item", values_to = "resp", keep = function(v) TRUE, na.rm = TRUE)
  testthat::expect_s3_class(out, "adlgraphs_freqs")
...
testthat::test_that("finalize_common_attrs attaches expected class and labels", {
  ...
  testthat::expect_true(inherits(out, "adlgraphs_freqs"))
  testthat::expect_equal(attr(out$n, "label"), "N")
  testthat::expect_equal(attr(out$pct, "label"), "Percent")
})
...
# testthat::test_that("survey Case D: grid completion occurs only when na.rm=TRUE", {
#   df <- label_vars(make_basic_df())

#   # Add unused level AND its value label to satisfy make_factor()
#   df$x2 <- factor(df$x2, levels = c("yes", "no", "skip"))
#   attr(df$x2, "labels") <- c(yes = "yes", no = "no", skip = "skip")

#   svy <- make_svy(df)

#   # na.rm = FALSE: no grid completion; 'skip' not fabricated
#   out_false <- get_freqs(
#     svy, x = c("x1", "x2"), group = grp,
#     names_to = "item", values_to = "resp",
#     na.rm = FALSE, drop_zero = FALSE
#   )
#   testthat::expect_false(any(out_false$resp == "skip"))

#   # na.rm = TRUE: grid completion may add zero-count 'skip' rows for x2
#   out_true <- get_freqs(
#     svy, x = c("x1", "x2"), group = grp,
#     names_to = "item", values_to = "resp",
#     na.rm = TRUE, drop_zero = FALSE
#   )

#   # Safely assert: if Q2 is present, 'skip' may appear with 0 count
#   if (any(out_true$item == "Q2. Color")) {
#     d2 <- subset(out_true, item == "Q2. Color")
#     if (any(d2$resp == "skip")) {
#       testthat::expect_true(all(d2$n[d2$resp == "skip"] == 0))
#     }
#   }
# })
...
testthat::test_that("function keep accepts predicate on response vector", {
  df <- tibble::tibble(resp = c("yes", "no", NA), n = c(1, 2, 3))
  out <- apply_keep_filter(df, values_col = "resp", keep = rlang::quo(function(x) x == "no"))
  testthat::expect_equal(out$resp, "no")
})

testthat::test_that("function keep can return TRUE scalar", {
  df <- tibble::tibble(resp = c("yes", "no", NA), n = c(1, 2, 3))
  out <- apply_keep_filter(df, values_col = "resp", keep = rlang::quo(function(x) TRUE))
...
testthat::test_that("logical keep of correct length is allowed", {
  df <- tibble::tibble(resp = c("yes", "no", NA), n = c(1, 2, 3))
  out <- apply_keep_filter(df, values_col = "resp", keep = rlang::quo(c(TRUE, FALSE, NA)))
  testthat::expect_equal(out$resp, "yes")
})
...
testthat::test_that("wrong-length masks error out", {
  df <- tibble::tibble(resp = c("yes", "no", NA), n = c(1, 2, 3))
  testthat::expect_error(
    apply_keep_filter(df, values_col = "resp", keep = rlang::quo(c(TRUE, FALSE))),
    "length nrow"
  )
  testthat::expect_error(
    apply_keep_filter(df, values_col = "resp", keep = rlang::quo(function(x) 1:3)),
    "logical"
  )
...
testthat::test_that("missing values_col is warned and skipped", {
  df <- tibble::tibble(resp = c("yes", "no", NA), n = c(1, 2, 3))
  testthat::expect_warning({
    out <- apply_keep_filter(df, values_col = "missing_col", keep = rlang::quo(c("yes")))
  })
