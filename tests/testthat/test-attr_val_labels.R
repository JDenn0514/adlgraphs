testthat::test_that("check an individual vector", {
  testthat::expect_equal(
    attr_val_labels(test_data$top), attr(test_data$top, "labels")
  )
})

testthat::test_that("check an individual vector, with data and x separate", {
  testthat::expect_equal(
    attr_val_labels("top", test_data), attr(test_data$top, "labels")
  )
})
...
testthat::test_that("check an individual vector, with data and x separate", {
  ...
  exp <- c("Strongly agree" = 1, "Somewhat agree" = 2, "Somewhat disagree" = 3, "Strongly disagree" = 4)
