...
This function adds a new attribute called `question_preface` based on
...
- custom:
  ...
  If a string is provided, sets the `question_preface` attribute
  manually. If nothing is provided, will use the existing variable label
  and value label by removing the value label from the variable label.
  See examples to see it in practice.
