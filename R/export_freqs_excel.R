...
export_freqs_excel <- function(
  ...
  if (append_to_existing && file.exists(file_name)) {
    ...
    print(paste("Loaded workbook with sheets:", paste(wb$get_sheet_names(), collapse = ", ")))
    ...
    for (freq in freq_list) {
      ...
      wide <- prepare_freq_for_excel(freq, show_genpop = show_genpop, decimals = decimals, wt = wt)
      ...
      if (!is.null(group_names)) {
          ...
          for (col in pct_cols) {
            wb <- wb %>%
              openxlsx2::wb_add_numfmt(
                ...
                numfmt = paste0("0.", paste(rep("0", decimals), collapse = ""), "%")
              )
          }
        }
        ...
        wb <- wb %>%
          openxlsx2::wb_add_border(
            dims = openxlsx2::wb_dims(rows = table_start_row:table_end_row, cols = table_start_col:table_end_col),
        ...
        if (!is.null(group_names) && !is.null(group_labels)) {
          ...
          for (i in seq_along(group_names)) {
            ...
            if (i == 1) {
              explanation_line <- paste0("Note: Top level headers represent ", group_label)
            } else if (i == 2) {
              explanation_line <- paste0("Note: Second level headers represent ", group_label)
            } else if (i == 3) {
              explanation_line <- paste0("Note: Third level headers represent ", group_label)
            } else {
              explanation_line <- paste0("Note: Level ", i, " headers represent ", group_label)
            }
          ...
          for (line in explanation_lines) {
            explanation_dims <- openxlsx2::wb_dims(rows = row_pointer, cols = 1:ncol(wide))
        ...
        wb <- wb %>%
          ...
          openxlsx2::wb_add_font(
            dims = openxlsx2::wb_dims(rows = row_pointer, cols = 1:length(header_row)),  # Fixed variable name
            bold = TRUE
          ) %>%
          openxlsx2::wb_add_fill(
            dims = openxlsx2::wb_dims(rows = row_pointer, cols = 1:length(header_row)),  # Fixed variable name
...
prepare_freq_for_excel <- function(
  ...
  if (is.null(group_names)) group_names <- character(0)  
  ...
  if (show_genpop && length(group_names) > 0) {
    ...
    names(genpop)[names(genpop) %in% c("n", "pct")] <- c("General Population_n", "General Population_pct")
    # reorder the columns
...
create_hierarchical_headers <- function(
  ...
  if (length(header_cols) == 0) return(list(wb = wb, rows_used = 0))
  ...
  wb <- wb %>%
    openxlsx2::wb_add_font(
      dims = openxlsx2::wb_dims(rows = row_start:(row_start + max_levels - 1), 
  ...
  for (level in 1:(max_levels - 1)) {  # Don't merge the bottom level
    ...
    while (i <= ncol(header_matrix)) {
      if (header_matrix[level, i] != "") {
        ...
        while (i <= ncol(header_matrix) && header_matrix[level, i] == current_value) {
          i <- i + 1
        }
        end_col <- i - 1
        ...
        if (end_col > start_col) {
          wb <- wb %>%
            openxlsx2::wb_merge_cells(
              sheet = sheet,
              dims = openxlsx2::wb_dims(rows = current_row, cols = start_col:end_col)
            )
        }
      } else {
        i <- i + 1
