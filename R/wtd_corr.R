# wtd_corr <- function(data, x, y, wts) {
#   x <- adlgraphs:::accept_string_or_sym({{ x }})
#   wts <- adlgraphs:::accept_string_or_sym({{ wts }})
#   y <- adlgraphs:::accept_string_or_sym({{ y }})
  
#   n <- length(data[[x]])
  
#   weights::wtd.cor(data[[x]], data[[y]], data[[wts]]) %>% 
#     as.vector() %>% 
#     setNames(c("correlation", "std.err", "t.value", "p.value")) %>% 
#     enframe() %>% 
...
#     ) %>% 
#     select(-c(t.value, std.err)) %>% 
#     relocate(p.value, .before = "stars")
# }
