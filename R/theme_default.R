...
#'   the graph.
#' @param base_line_size Base size for line ggplot2::elements.
#' @param base_rect_size Base size for rect ggplot2::elements.
...
theme_default <- function(
  ...
  if (!is.null(panel_spacing_x)) {
    panel_spacing_x <- ggplot2::unit(panel_spacing_x, "pt")
  }

  # set the vertical panel spacing
  if (!is.null(panel_spacing_y)) {
    panel_spacing_y <- ggplot2::unit(panel_spacing_y, "pt")
  }
