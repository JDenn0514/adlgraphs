  ...
  if (!is.null(labels)) {
    if (is.character(labels)) {
      ...
      out <- out[c(
        "variables",
        "labels",
        names(out[!names(out) %in% c("variables", "labels")])
      )]
...
get_loadings.data.frame <- function(
  ...
  if (!missing(cols)) {
    data <- data %>% dplyr::select(tidyselect::all_of(group_names), {{ cols }})
  }
  ...
  if ("labels" %in% names(out)) {
    ...
    load_names <- names(out[
      !names(out) %in%
        c(group_names, "variables", "labels", "communality", "uniqueness")
    ])
  } else {
    # get the names of the loading variables
    load_names <- names(out[
      !names(out) %in% c(group_names, "variables", "communality", "uniqueness")
    ])
  }
