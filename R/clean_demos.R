  ...
  if (
    ...
    df <- df %>%
      dplyr::mutate(
        ...
        race_f = refactor(
          race_f,
          c("White", "Black", "Hispanic", "AAPI", "Multi/Other")
        ) %>%
    ...
    if ("start_date" %in% colnames(df)) {
      df <- df %>%
        dplyr::mutate(
          ...
          birth_year = (as.numeric(current_year) - age_n) %>%
            # set the label
    ...
    df <- df %>%
      dplyr::mutate(
        ...
        progressive_f = make_factor(progressive) %>%
          structure(
            label = "Politically, would you say you identify as a Progressive?"
          ),
    ...
    df <- df %>%
      dplyr::mutate(
        ...
        religpew_evan_f = dplyr::case_when(
          ...
          religpew %in% c(1, 5) & born_again != 1 ~
