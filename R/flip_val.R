...
flip_val <- function(x, label = NULL) {
  ...
  if (!is.null(attr_val_labels(x))) {
    ...
    attr(x, "transformation") <- paste0("Flipped the valance of `", x_name, "` so it is now negative by updating the label and value labels accordingly.")
  } else {
    # if there aren't any labels then just set the transformation attribute
    attr(x, "transformation") <- paste0("Flipped the valance of `", x_name, "` so it is now negative by updating the label accordingly.")
  }
